...
PoolItem::PoolItem( const QString &text, PoolView *poolView, HostInfoManager *m )
  : QGraphicsItemGroup(), mHostInfo( 0 ), mHostInfoManager( m ),
...
void PoolItem::update( const Job &job )
{
    ...
    if ( newJob )
    {
      ...
      m_jobLines[ job.jobId() ] = new QGraphicsLineItem();
      scene()->addItem(m_jobLines[ job.jobId() ]);
